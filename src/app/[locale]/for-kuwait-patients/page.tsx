...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = isArabic
    ? 'الخدمات الطبية للمرضى من الكويت - شفاء الهند'
    : 'Medical Services for Kuwait Patients - Shifa AlHind';

  const description = isArabic
    ? 'خدمات رعاية صحية متخصصة للمرضى من جميع مدن الكويت (مدينة الكويت، حولي، السالمية، الفروانية) في أفضل مستشفيات الهند. وفر 60-70٪ مع مستشفيات معتمدة من JCI، دعم عربي كامل، مساعدة في التأشيرة، وخدمات النقل.'
    : 'Specialized healthcare services for patients from all Kuwait cities (Kuwait City, Hawalli, Salmiya, Farwaniya) in top Indian hospitals. Save 60-70% with JCI-accredited hospitals, complete Arabic support, visa assistance, and transportation services.';

  const keywords = [
    ...seoKeywords.homepage,
    'Kuwait patients India',
    'Kuwait City to India medical tourism',
    'Hawalli medical tourism',
    'Salmiya patients India',
    'Farwaniya to Bangalore healthcare',
    'Kuwaiti patients treatment India',
    'Kuwait medical tourism',
    'Kuwait to Bangalore healthcare',
    'Kuwait patients India hospitals',
    'medical visa Kuwait to India',
    'healthcare for Kuwait residents',
  ];
  ...
  return generateFullMetadata({
    ...
    canonical: `/${locale}/for-kuwait-patients`,
    ogType: 'website',
  });
}

export default async function KuwaitPatientsPage({ params }: PageProps) {
  const { locale } = await params;
  const isArabic = locale === 'ar';

  const breadcrumbItems = [
    { name: isArabic ? 'الرئيسية' : 'Home', url: '/' },
    { name: isArabic ? 'لمرضى الكويت' : 'For Kuwait Patients', url: '/for-kuwait-patients' },
  ];
  ...
  return (
    <>
      ...
      <section className="bg-gradient-to-r from-primary to-primary-dark text-white py-20">
        <div className="container mx-auto px-4">
          <h1 className="text-4xl md:text-5xl font-bold mb-6">
            {isArabic
              ? 'مرحباً بمرضى دولة الكويت'
              : 'Welcome Kuwait Patients to World-Class Healthcare in India'}
          </h1>
          <p className="text-xl mb-8">
            {isArabic
              ? 'احصل على رعاية صحية عالمية المستوى في الهند بتكلفة 60-70٪ أقل من الكويت'
              : 'Get world-class healthcare in India at 60-70% lower cost than Kuwait'}
          </p>
      ...
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12">
            {isArabic
              ? 'لماذا يختار مرضى الكويت الهند للعلاج؟'
              : 'Why Kuwait Patients Choose India for Treatment'}
          </h2>
          <div className="grid md:grid-cols-3 gap-8">
            <div className="bg-white p-6 rounded-lg shadow-md">
              ...
              <p className="text-gray-600">
                {isArabic
                  ? 'وفر 60-70٪ على جميع الإجراءات الطبية مقارنة بأسعار الكويت. جراحة القلب: 10,000 - 15,000 دولار (مقابل 45,000+ دولار في الكويت)'
                  : 'Save 60-70% on all medical procedures compared to Kuwait prices. Heart surgery: $10,000-15,000 (vs $45,000+ in Kuwait)'}
              </p>
            </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
              ...
              <p className="text-gray-600">
                {isArabic
                  ? 'فقط 4-5 ساعات طيران من الكويت إلى بنغالور. رحلات مباشرة على الخطوط الجوية الكويتية وإنديجو. لا حاجة لرحلات طويلة مرهقة.'
                  : 'Only 4-5 hours flight from Kuwait to Bangalore. Direct flights on Kuwait Airways and IndiGo. No long exhausting journeys required.'}
              </p>
            </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
              ...
              <p className="text-gray-600">
                {isArabic
                  ? 'نعمل فقط مع مستشفيات حاصلة على اعتماد JCI و NABH الدولي. نفس معايير الجودة مثل أفضل مستشفيات الكويت.'
                  : 'We work only with JCI and NABH internationally accredited hospitals. Same quality standards as top Kuwait hospitals.'}
              </p>
            </div>
      ...
      {/* All 4 Kuwait Cities Section */}
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-4">
            <div className="inline-block bg-primary/10 text-primary px-4 py-2 rounded-full text-sm font-semibold mb-2">
              {isArabic ? '4 مدن من الكويت ✓' : '4 Kuwait Cities Covered ✓'}
            </div>
          </div>
          <h2 className="text-3xl font-bold text-center mb-4">
            {isArabic ? 'علاج طبي من جميع مدن الكويت' : 'Medical Treatment from All Kuwait Cities'}
          </h2>
          <p className="text-center text-gray-600 mb-12 max-w-2xl mx-auto">
            {isArabic
              ? 'نخدم المرضى من جميع مدن الكويت الرئيسية. اختر مدينتك لمعرفة الخيارات المتاحة'
              : 'We serve patients from all major Kuwait cities. Select your city to explore treatment options'}
          </p>
      ...
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12">
            {isArabic
              ? 'العلاجات الأكثر شعبية لمرضى الكويت'
              : 'Most Popular Treatments for Kuwait Patients'}
          </h2>
      ...
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12">
            {isArabic ? 'خدماتنا لمرضى الكويت' : 'Our Services for Kuwait Patients'}
          </h2>
